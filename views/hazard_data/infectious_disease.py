...
InfOutbreak_data_file = 'InfOutbreak_data.json'
...
# Function to load data from the InfOutbreak_data.json file
def load_InfOutbreak_data():
    if os.path.exists(InfOutbreak_data_file):
        with open(InfOutbreak_data_file, 'r') as file:
            try:
                data = json.load(file)
                return data
            except json.JSONDecodeError:
...
# Function to save data to the InfOutbreak_data.json file
def save_data(page_key, data):
    try:
        # Load existing data from the InfOutbreak_data.json file
        if os.path.exists(InfOutbreak_data_file):
            with open(InfOutbreak_data_file, 'r') as file:
        ...
        with open(InfOutbreak_data_file, 'w') as file:
...
def show():
    ...
    InfOutbreak_data = load_InfOutbreak_data()

    st.markdown("""
    <p>A pandemic is an epidemic that occurs over a wide geographic area, often global. 
    A pandemic results when a new microorganism (or disease condition) emerges that this pathogenic 
    for humans but to which humans have no immunity or prior protection. Thus, an epidemic occurs and 
    the number of cases substantially exceeds the number of expended cases over a given period of time. 
    Pandemics generally refer to infectious diseases that spread efficiently from person to person across 
    the globe although the term may be used to describe medical condition with other risk factors, such as 
    chronic illnesses like cardiovascular diseases (Madhav et al., 2017).</p>
    ...
    st.text_area("", "The proxy scenario used to predict the impacts of an infectious disease outbreak in South Dakota is the following: COVID-19: In late 2019, a severe respiratory disease known as COVID-19, caused by the novel coronavirus, began to spread rapidly around the world. By early 2020, COVID-19 had been detected in over 100 countries, including the United States. In response to the growing pandemic, Executive Order 2020-04 declared South Dakota to be in a State of Emergency (South Dakota Secretary of State, 2020).")
...
# Calculate and display Human Impact Score
    calculate_human_impact_score(InfOutbreak_data)
    ...
    calculate_healthcare_service_impact_score(InfOutbreak_data)
...
def handle_section(section_title, baseline_label, increase_label, default_explanation=""):
    ...
    InfOutbreak_increase = st.number_input(increase_label, value=0, key=f"{section_title}_InfOutbreak_increase")
    
    # Calculate Magnitude Score
    magnitude_score = calculate_magnitude_score(baseline_value, InfOutbreak_increase)
    ...
    save_data(section_title, {
        ...
        "InfOutbreak_increase": InfOutbreak_increase,
...
def calculate_human_impact_score(InfOutbreak_data):
    """
    Function to calculate Human Impact Score.
    """
    # Retrieve the final scores for each human impact section
    mortality_score = InfOutbreak_data.get("Mortality_final", {}).get("final_score", "Not Calculated")
    ems_transports_score = InfOutbreak_data.get("EMS Transports_final", {}).get("final_score", "Not Calculated")
    ed_visits_score = InfOutbreak_data.get("ED Visits_final", {}).get("final_score", "Not Calculated")
    primary_care_office_visits_score = InfOutbreak_data.get("Primary Care Office Visits_final", {}).get("final_score", "Not Calculated")
    trauma_center_injuries_score = InfOutbreak_data.get("Trauma Center Injuries_final", {}).get("final_score", "Not Calculated")
    mental_health_score = InfOutbreak_data.get("Mental Health_final", {}).get("final_score", "Not Calculated")
   ...
   # Save the Human Impact Score to InfOutbreak_data.json
    InfOutbreak_data["Human_Impact_Score"] = human_impact_score
    # Save back to the InfOutbreak_data.json file
...
def handle_healthcare_section(section_title, baseline_label, loss_label, increase_label, default_explanation=""):
    ...
    InfOutbreak_loss_value = st.number_input(loss_label, value=0, key=f"{section_title}_InfOutbreak_loss")

    # Logic for ED Services: Hazard-Related Increase = ED Visits / 4.5
    if section_title == "ED Services":
        InfOutbreak_data = load_InfOutbreak_data()  # Load data from ED Visits section
        InfOutbreak_increase_visits = InfOutbreak_data.get("ED Visits", {}).get("InfOutbreak_increase", 0)
        InfOutbreak_increase_value = InfOutbreak_increase_visits / 4.5
        st.markdown(f"**{increase_label}:** {InfOutbreak_increase_value}")

    # Logic for Outpatient Services: Hazard-Related Increase = Office Visits / 20
    elif section_title == "Outpatient Services":
        InfOutbreak_data = load_InfOutbreak_data()  # Load data from Primary Care Office Visits section
        InfOutbreak_increase_visits = InfOutbreak_data.get("Primary Care Office Visits", {}).get("InfOutbreak_increase", 0)
        InfOutbreak_increase_value = InfOutbreak_increase_visits / 20
        st.markdown(f"**{increase_label}:** {InfOutbreak_increase_value}")

    # Logic for Hospital Beds: Editable by user
    elif section_title == "Hospital Beds":
        # Hazard-Related Increase (editable by user)
        InfOutbreak_increase_value = st.number_input(increase_label, value=0, key=f"{section_title}_InfOutbreak_increase")
    ...
    # Logic for Anxillary Services: Editable by user
        ...
        InfOutbreak_data = load_InfOutbreak_data()  # Load data from Primary Care Office Visits section
        InfOutbreak_increase_visits = InfOutbreak_data.get("Trauma Center Injuries", {}).get("InfOutbreak_increase", 0)
        InfOutbreak_increase_value = InfOutbreak_increase_visits 
        st.markdown(f"**{increase_label}:** {InfOutbreak_increase_value}")
    ...
    # Calculate Magnitude Score
    magnitude_score = calculate_magnitude_score_healthcare(baseline_value, InfOutbreak_increase_value, InfOutbreak_loss_value)
    st.markdown(f"**Magnitude Score:** {magnitude_score}")
    st.markdown("""
        <ul>
            <li>0: No change from baseline</li>
    ...
    save_data(section_title, {
        "baseline": baseline_value,
        "InfOutbreak_increase": InfOutbreak_increase_value,
        "InfOutbreak_loss_value": InfOutbreak_loss_value,
        "magnitude_score": magnitude_score
    })
...
def calculate_healthcare_service_impact_score(InfOutbreak_data):
    """
    Function to calculate Healthcare Service Impact Score.
    """
    outpatient_score = InfOutbreak_data.get("Outpatient Services_final", {}).get("final_score", "Not Calculated")
    ed_services_score = InfOutbreak_data.get("ED Services_final", {}).get("final_score", "Not Calculated")
    hospital_beds_score = InfOutbreak_data.get("Hospital Beds_final", {}).get("final_score", "Not Calculated")
    ancillary_services_score = InfOutbreak_data.get("Ancillary Services_final", {}).get("final_score", "Not Calculated")
    trauma_units_score = InfOutbreak_data.get("Trauma Units_final", {}).get("final_score", "Not Calculated")
    mental_health_score = InfOutbreak_data.get("Mental Health Services_final", {}).get("final_score", "Not Calculated")
    ...
    #save InfOutbreak_data
    InfOutbreak_data["Healthcare_Service_Impact_Score"] = healthcare_service_impact_score
