...
# Function to load data from the utility_data.json file
def load_utility_data():
    if os.path.exists(utility_data_file):
        with open(utility_data_file, 'r') as file:
            try:
                data = json.load(file)
                return data
            except json.JSONDecodeError:
...
# Function to save data to the utility_data.json file
def save_data(page_key, data):
    try:
        # Load existing data from the utility_data.json file
        if os.path.exists(utility_data_file):
            with open(utility_data_file, 'r') as file:
        ...
        with open(utility_data_file, 'w') as file:
...
def show():
    ...
    prefill_form()
    # Load utility data from file
    utility_data = load_utility_data()

    st.markdown("""
    <p>Utility interruption is defined as any disruption or loss of a public service which includes, but is not limited to: electrical service, 
    potable water, and natural gas caused by disruption of power transmission caused by accident, sabotage, natural hazards, or equipment failure 
    (also referred to as a utility failure or utility outage). A significant utility interruption is defined as any incident of a long duration, 
    which would require the involvement of the local and/or State emergency management organizations to coordinate provision of food, water, 
    heating, cooling, and shelter (“Utility Interruption,” 2020). </p>
    ...
    reflect utility impacts in your jurisdiction. The impacts should reflect the worst-case reasonable scenario.</p>
    ...
    st.text_area("", "The proxy scenario used to predict the impacts of a widespread utility interruption in South Dakota is the following: A particularly dangerous storm system known as a derecho moved through the Northern Plains during the late afternoon and early evening hours of Thursday, May 12th, including eastern South Dakota and west central Minnesota. Widespread impacts resulted from this storm, including uprooted or snapped trees, significant property damage, power outages, tipped high-profile vehicles, as well as injuries and a couple fatalities across the region (NOAA, 2022).")
...
# Calculate and display Human Impact Score
    calculate_human_impact_score(utility_data)
    ...
    calculate_healthcare_service_impact_score(utility_data)
...
def handle_section(section_title, baseline_label, increase_label, default_explanation=""):
    ...
    utility_increase = st.number_input(increase_label, value=0, key=f"{section_title}_utility_increase")
    
    # Calculate Magnitude Score
    magnitude_score = calculate_magnitude_score(baseline_value, utility_increase)
    ...
    save_data(section_title, {
        ...
        "utility_increase": utility_increase,
...
def calculate_human_impact_score(utility_data):
    """
    Function to calculate Human Impact Score.
    """
    # Retrieve the final scores for each human impact section
    mortality_score = utility_data.get("Mortality_final", {}).get("final_score", "Not Calculated")
    ems_transports_score = utility_data.get("EMS Transports_final", {}).get("final_score", "Not Calculated")
    ed_visits_score = utility_data.get("ED Visits_final", {}).get("final_score", "Not Calculated")
    primary_care_office_visits_score = utility_data.get("Primary Care Office Visits_final", {}).get("final_score", "Not Calculated")
    trauma_center_injuries_score = utility_data.get("Trauma Center Injuries_final", {}).get("final_score", "Not Calculated")
    mental_health_score = utility_data.get("Mental Health_final", {}).get("final_score", "Not Calculated")
   ...
   # Save the Human Impact Score to utility_data.json
    utility_data["Human_Impact_Score"] = human_impact_score
    # Save back to the utility_data.json file
...
def handle_healthcare_section(section_title, baseline_label, loss_label, increase_label, default_explanation=""):
    ...
    utility_loss_value = st.number_input(loss_label, value=0, key=f"{section_title}_utility_loss")

    # Logic for ED Services: Hazard-Related Increase = ED Visits / 4.5
    if section_title == "ED Services":
        utility_data = load_utility_data()  # Load data from ED Visits section
        utility_increase_visits = utility_data.get("ED Visits", {}).get("utility_increase", 0)
        utility_increase_value = utility_increase_visits / 4.5
        st.markdown(f"**{increase_label}:** {utility_increase_value}")

    # Logic for Outpatient Services: Hazard-Related Increase = Office Visits / 20
    elif section_title == "Outpatient Services":
        utility_data = load_utility_data()  # Load data from Primary Care Office Visits section
        utility_increase_visits = utility_data.get("Primary Care Office Visits", {}).get("utility_increase", 0)
        utility_increase_value = utility_increase_visits / 20
        st.markdown(f"**{increase_label}:** {utility_increase_value}")

    # Logic for Hospital Beds: Editable by user
    elif section_title == "Hospital Beds":
        # Hazard-Related Increase (editable by user)
        utility_increase_value = st.number_input(increase_label, value=0, key=f"{section_title}_utility_increase")
    ...
    # Logic for Anxillary Services: Editable by user
        ...
        utility_data = load_utility_data()  # Load data from Primary Care Office Visits section
        utility_increase_visits = utility_data.get("Trauma Center Injuries", {}).get("utility_increase", 0)
        utility_increase_value = utility_increase_visits 
        st.markdown(f"**{increase_label}:** {utility_increase_value}")
    ...
    # Logic for Mental Health Services: Editable by user
        ...
        utility_increase_value = 0

    # Calculate Magnitude Score
    magnitude_score = calculate_magnitude_score_healthcare(baseline_value, utility_increase_value, utility_loss_value)
    st.markdown(f"**Magnitude Score:** {magnitude_score}")
    st.markdown("""
        <ul>
            <li>0: No change from baseline</li>
    ...
    save_data(section_title, {
        "baseline": baseline_value,
        "utility_increase": utility_increase_value,
        "utility_loss_value": utility_loss_value,
        "magnitude_score": magnitude_score
    })
...
def calculate_healthcare_service_impact_score(utility_data):
    """
    Function to calculate Healthcare Service Impact Score.
    """
    outpatient_score = utility_data.get("Outpatient Services_final", {}).get("final_score", "Not Calculated")
    ed_services_score = utility_data.get("ED Services_final", {}).get("final_score", "Not Calculated")
    hospital_beds_score = utility_data.get("Hospital Beds_final", {}).get("final_score", "Not Calculated")
    ancillary_services_score = utility_data.get("Ancillary Services_final", {}).get("final_score", "Not Calculated")
    trauma_units_score = utility_data.get("Trauma Units_final", {}).get("final_score", "Not Calculated")
    mental_health_score = utility_data.get("Mental Health Services_final", {}).get("final_score", "Not Calculated")
    ...
    #save utility_data
    utility_data["Healthcare_Service_Impact_Score"] = healthcare_service_impact_score
